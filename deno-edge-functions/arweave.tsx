...
const movespace_key = Deno.env.get('MOVESPACE_API_KEY');
...
router
  .post("/", async (context) => {
    ...
    context.response.body = {content: "Hello0!"};
  })
  ...
  .post("/upload", async (context) => {
    ...
    const tags = {"App-Name": "Arweave-based-GPT-Bot"};
    // call faas.movespace.xyz to upload to arweave by escrow acct.
    // return tx_id
    const resp = 
        await fetch('https://faas.movespace.xyz/api/v1/run?name=ArweaveUploader&func_name=send_tx', {
            method: 'POST',
            headers: {
            'Content-Type': 'application/json',
            },
            body: JSON.stringify({"params": [movespace_key, content, {}]}),
        })
    const resp_jsoned = await resp.json()
    await console.log("result", resp_jsoned);
    context.response.body = { tx_id: resp_jsoned.result };
  })
